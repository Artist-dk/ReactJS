...
export default function StudySnap() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.info(lSidebarContContent)
      } catch (error) {
  ...
  const [chatBody, setChatBody] = useState([1,2,31,2,31,2,31,2,31,2,3]);
  useEffect(() => {
    const fetchData = async () => {
      try {
  ...
  return (
    ...
    (function () {
      ...
      for (let i = 0; i < chatBody.length; i++) {
        const msg = chatBody[i];
        if(msg.sender_firstName == 'Digambar') {
          messageElement.push(
          <div key={i} className="msg-cont-b">
          ...
          messageElement.push(
          <div key={i} className="msg-cont-a">
            <div className="msg-body">
                <div className="msg-bg">
                  <div className="msg-text">{msg.message}</div>
                </div>
